...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> degreesequece = new ArrayList<Integer>();
		List<Integer> in_neighbors = new ArrayList<>();
		List<Integer> out_neighbors = new ArrayList<>();
		for (int i=0; i<getNumVertices(); i++){
			int num_neighbors = 0;
			out_neighbors = getNeighbors(i);
				for (Integer j : out_neighbors){
					num_neighbors+=1;
				}
			in_neighbors = getInNeighbors(i);
				for (Integer j : in_neighbors){
					num_neighbors+=1;
				}
			degreesequece.add(num_neighbors);
		}
		Collections.sort(degreesequece);
		Collections.reverse(degreesequece);
		return degreesequece;
	}
